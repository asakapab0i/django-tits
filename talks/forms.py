...
class TalkForm(forms.ModelForm):
    ...
    def clean_when(self):
        ...
        pycon_start = datetime.datetime(2014,
                4,11).replace(tzinfo=utc)
        pycon_end = datetime.datetime(2014, 4, 13,
                17).replace(tzinfo=utc)

        if not pycon_start < when < pycon_end:
            raise forms.ValidationError("'when' is outside of PyCon.")
        return when
